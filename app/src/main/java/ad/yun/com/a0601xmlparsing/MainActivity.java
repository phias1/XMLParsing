package ad.yun.com.a0601xmlparsing;
...
public class MainActivity extends AppCompatActivity {
    ...
    //anonymous - 익명(이름없는)클래스의 객체 생성
    //객체는 1개만 생성된다.
    //handleMessage가 콜백 메소드(이벤트가 발생하면 자동으로 호출되는 메소드) 이므로 객체를 1개만 만들면 된다.
    ...
    //스레드에서 run은 객체를 생성한 후 start()를 직접 호출해서 실행하는 메소드이므로 여러번 사용하려면 객체를
    //여러 개 만들어야 하는 경우가 있다.
    //스레드는 한 번 만들고 종료되면 재사용이 안되는 객체이다.
    //매번 새로 만들어서 사용해야 하기 때문에 별도로 클래스를 만들어서 사용하거나 이벤트가 발생할 때 마다
    //익명 객체를 만들어서 사용해야 한다.
        ...
        public void run(){
            ...
            try{
                ...
                for(int i = 0; i < items1.getLength(); i = i + 1){
                    ...
                    //wf 태그는 전체 날씨를 나타내는 태그가 한 개 더 있으므로 + 1을 해서
                    //가져와야 한다.
                ...
                //Log.e("날씨", weathers.toString());

                //핸들러에게 메시지 전송
                handler.sendEmptyMessage(0);
            }catch (Exception e){
                Log.e("xml예외", e.getMessage());
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        button.setOnClickListener(new Button.OnClickListener(){
            ...
            public void onClick(View v) {
                progressDialog = ProgressDialog.show(MainActivity.this, "날씨 데이터", "가져오는 중");
                //스레드 생성 및 시작
